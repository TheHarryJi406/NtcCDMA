...
String pageName = "rCollectionCenterWise.jsp";
...
if (menuAccessDao.checkMenuAccessForUser(userName, pageName)==false) {
...
// 		System.out.println(transType);
		...
		if(transType.equals("all")){
			tempTransType="";
				...
				tempTransType="&transType="+transType;
			}
		...
		String url="../WebViewerExample/frameset?__report=cdmaReport/collCenterWiseCollection.rptdesign"+tempCollCen+"&frmDate="+fromDate+"&toDate="+toDate+"&createdBy="+user+tempAccCen+tempRegion+tempTransType;
		url = URLEncoder.encode(url);
		response.sendRedirect("rCollectionCenterWise.jsp?&url="+url);
		return;
		}
...
<h2>Collection Center Wise Total Collection</h2>
<br><br>

<!-- <div style="text-align: right;"><a href="region.jsp?mode=add" >Add new</a></div> -->
<div>
	<form id="form1" action="rCollectionCenterWise.jsp" method="post">
		<table class="xform">
			<tr>
				<%		
			...
			<tr><td>Transaction Type:</td>
				<td>
					<%String selTrans = request.getParameter("transtype");
					selTrans = selTrans == null ? "" : selTrans; %>
					<select name="transtype" id="transtype" class="transtype" title="Select Trans Type"  STYLE="width: 250px" >
						<option value="">Select </option>
						<option value="all" <%if (selTrans.equals("all")) {out.write("selected");	}%> >All </option>
						<option value="PMTB"  <%if (selTrans.equals("PMTB")) {out.write("selected");	}%>  >Bill Payment</option>
						<option value="CSIA" <%if (selTrans.equals("CSIA")) {out.write("selected");	}%> >Cash Sales</option>
						<option value="DEPS" <%if (selTrans.equals("DEPS")) {out.write("selected");	}%> >Deposit</option>
					</select>
				</td>
			</tr> 
...
// 							System.out.println(selCCCen);
				...
				%>
					...
					<td>
						<select  name="collCen" id="collCen"  STYLE="width: 250px" class="collCen" title="Select Collection Center" >
							<option value="">Select </option>
...
// 							System.out.println("sample----"+request.getParameter("accCen"));
				...
				%>
					<td>Collection Center:</td>
					<td>
						<select  name="collCen" id="collCen" class="collCen"  STYLE="width: 250px" title="Select Collection Center" >
							<option value="">Select </option>
							<option value="all" <%if (selCCCen.equals("all")) {out.write("selected");	}%> >All </option>
						<%
							for (Map m : list1) {
...
// 						}
			...
			<!-- 			for user level 3 -->
				...
				%>
					<td>Collection Center:</td>
					<td>
						<select  name="collCen" id="collCen" class="collCen" title="Select Collection Center" STYLE="width: 250px" >
							<option value="">Select </option>
							<option value="all" <%if (selCCCen.equals("all")) {out.write("selected");	}%> >All </option>
						<%
							for (Map m : list1) {
