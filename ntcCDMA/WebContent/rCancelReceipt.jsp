...
String pageName = "rCancelReceipt.jsp";
...
if (menuAccessDao.checkMenuAccessForUser(userName, pageName)==false) {
	...
	if(request.getParameter("ok")!=null){
		...
		String url="../WebViewerExample/frameset?__report=cdmaReport/cancelledReceipts.rptdesign"+tempCollCen+"&frmDate="+fromDate+"&toDate="+toDate+"&createdBy="+user+tempAccCen+tempRegion+tempClrk;
		url = URLEncoder.encode(url);
		response.sendRedirect("rCancelReceipt.jsp?&url="+url);
		return;
		}
...
<h2>List of Cancelled Receipts</h2>
<br><br>

<!-- <div style="text-align: right;"><a href="region.jsp?mode=add" >Add new</a></div> -->
<div>
	<form id="form1" action="rCancelReceipt.jsp" method="post">
		<table class="xform">
			<tr>
				<%		
			...
			<%-- <tr><td>Transaction Type:</td>
				<td>
					...
					<select name="transtype" id="transtype" class="transtype" title="Select Trans Type"  STYLE="width: 250px" >
						...
						<option value="DEB"  <%if (selTrans.equals("DEB")) {out.write("selected");	}%>  >Debtor</option>
						<option value="ADV" <%if (selTrans.equals("ADV")) {out.write("selected");	}%> >Advance</option>
						<option value="DEP" <%if (selTrans.equals("DEP")) {out.write("selected");	}%> >Deposit</option>
					</select>
				</td>
			</tr> --%>
